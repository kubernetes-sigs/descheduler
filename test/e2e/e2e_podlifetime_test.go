...
func podLifeTimeTransitionPolicy(args *podlifetime.PodLifeTimeArgs, evictorArgs *defaultevictor.DefaultEvictorArgs) *apiv1alpha2.DeschedulerPolicy {
...
func TestPodLifeTime_FailedPods(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			job := initTransitionTestJob(tc.name, testNamespace.Namespace)
			...
			tc.args.Namespaces = &api.Namespaces{
				Include: []string{testNamespace.Name},
			}

			deschedulerPolicyConfigMapObj, err := deschedulerPolicyConfigMap(podLifeTimeTransitionPolicy(tc.args, evictorArgs))
			if err != nil {
				t.Fatalf("Error creating %q CM: %v", deschedulerPolicyConfigMapObj.Name, err)
			}

			t.Logf("Creating %q policy CM with PodLifeTime configured...", deschedulerPolicyConfigMapObj.Name)
...
func TestPodLifeTime_SucceededPods(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			job := initTransitionSucceededJob(tc.name, testNamespace.Namespace)
			t.Logf("Creating job %s in %s namespace", job.Name, job.Namespace)
			jobClient := clientSet.BatchV1().Jobs(testNamespace.Name)
			if _, err := jobClient.Create(ctx, job, metav1.CreateOptions{}); err != nil {
				t.Fatalf("Error creating Job %s: %v", tc.name, err)
...
func initTransitionSucceededJob(name, namespace string) *batchv1.Job {
	...
	podSpec.Containers[0].Command = []string{"/bin/true"}
