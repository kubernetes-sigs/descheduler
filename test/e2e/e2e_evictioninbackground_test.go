...
func TestLiveMigrationInBackground(t *testing.T) {
	...
	deschedulerDeploymentObj.Spec.Template.Spec.Containers[0].Args = []string{"--policy-config-file", "/policy-dir/policy.yaml", "--descheduling-interval", "10s", "--v", "4"}
	...
	deschedulerPodName = waitForDeschedulerPodRunning(t, ctx, kubeClient, deschedulerDeploymentObj.Namespace)
	...
	t.Logf("Update %q policy for unlimited evictions", deschedulerPolicyConfigMapObj.Name)
	policy.MaxNoOfPodsToEvictPerNamespace = nil
	deschedulerPolicyConfigMapObj, err = deschedulerPolicyConfigMap(policy)
	if err != nil {
