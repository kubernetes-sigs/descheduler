...
func (rs *DeschedulerServer) AddFlags(fs *pflag.FlagSet) {
	...
	fs.IntVar(&rs.MaxNoOfPodsToEvictPerNode, "max-no-pods-to-evict", rs.MaxNoOfPodsToEvictPerNode, "Limits the maximum number of pods to be evicted per node by descheduler")
}
