...
func RemoveDuplicatePods(ds *options.DeschedulerServer, strategy api.DeschedulerStrategy, policyGroupVersion string, nodes []*v1.Node, nodepodCount nodePodEvictedCount) {
	...
	deleteDuplicatePods(ds.Client, ds.AnnotationsPrefix, policyGroupVersion, nodes, ds.DryRun, nodepodCount, ds.MaxNoOfPodsToEvictPerNode)
}

// deleteDuplicatePods evicts the pod from node and returns the count of evicted pods.
func deleteDuplicatePods(client clientset.Interface, annotationsPrefix string, policyGroupVersion string, nodes []*v1.Node, dryRun bool, nodepodCount nodePodEvictedCount, maxPodsToEvict int) int {
	podsEvicted := 0
	for _, node := range nodes {
		glog.V(1).Infof("Processing node: %#v", node.Name)
		dpm := ListDuplicatePodsOnANode(client, annotationsPrefix, node)
...
func ListDuplicatePodsOnANode(client clientset.Interface, annotationsPrefix string, node *v1.Node) DuplicatePodsMap {
	pods, err := podutil.ListEvictablePodsOnNode(client, annotationsPrefix, node)
	if err != nil {
		return nil
	}
