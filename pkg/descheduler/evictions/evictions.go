	...
	for uid, item := range erc.requests {
		if item.assumed {
			...
			if requestAgeSeconds > erc.assumedRequestTimeoutSeconds {
				klog.V(4).Infof("Assumed eviction request in background timed out after %vs for %v, deleting", erc.assumedRequestTimeoutSeconds, klog.KObj(item.pod))
				erc.deleteItem(uid)
			}
		}
	}
...
func (pe *PodEvictor) WaitForHandlersSync(ctx context.Context) error {
