...
func NewPodEvictor(
	...
	if features.DefaultFeatureGate.Enabled(features.EvictionsInBackground) {
			...
			cache.ResourceEventHandlerFuncs{
					...
					if _, exists := pod.Annotations[EvictionRequestAnnotationKey]; exists {
						if _, exists := pod.Annotations[EvictionInProgressAnnotationKey]; exists {
							...
							if pod.Status.Phase != v1.PodSucceeded && pod.Status.Phase != v1.PodFailed {
								klog.V(0).Infof("Eviction in background detected. Adding pod %q to the cache.", klog.KObj(pod))
				...
				UpdateFunc: func(oldObj, newObj interface{}) {
					...
					if newPod.Status.Phase == v1.PodSucceeded || newPod.Status.Phase == v1.PodFailed {
						klog.V(0).Infof("Pod with eviction in background completed. Removing pod %q from the cache.", klog.KObj(newPod))
					...
					if _, exists := newPod.Annotations[EvictionInProgressAnnotationKey]; !exists {
						...
						if _, exists := oldPod.Annotations[EvictionInProgressAnnotationKey]; exists {
							...
							// operation.
							klog.V(0).Infof("Eviction in background canceled (%q annotation removed). Removing pod %q from the cache.", EvictionInProgressAnnotationKey, klog.KObj(newPod))
							if err := erCache.deletePod(newPod); err != nil {
								// If the deletion fails the cache may block eviction
								klog.ErrorS(err, "Unable to delete updated pod from cache", "pod", newPod)
								return
				...
				DeleteFunc: func(obj interface{}) {
					...
					klog.V(0).Infof("Pod with eviction in background deleted/evicted. Removing pod %q from the cache.", klog.KObj(pod))
		...
		go wait.UntilWithContext(ctx, erCache.cleanCache, options.evictionRequestsCacheResyncPeriod)
...
func (pe *PodEvictor) EvictPod(ctx context.Context, pod *v1.Pod, opts EvictOptions) error {
	...
	if pe.dryRun {
		...
		klog.V(0).InfoS("Evicted pod", "pod", klog.KObj(pod), "reason", opts.Reason, "strategy", opts.StrategyName, "node", pod.Spec.NodeName, "profile", opts.ProfileName)
		reason := opts.Reason
		if len(reason) == 0 {
		...
		if pod.Annotations != nil {
			...
			if _, exists := pod.Annotations[EvictionRequestAnnotationKey]; exists {
				...
				if apierrors.IsTooManyRequests(err) && strings.Contains(err.Error(), EvictionInBackgroundErrorText) {
					...
					if pod.Status.Phase == v1.PodSucceeded || pod.Status.Phase == v1.PodFailed {
						klog.V(0).InfoS("Ignoring eviction of a completed/failed pod", "pod", klog.KObj(pod))
						return true, nil
					}
