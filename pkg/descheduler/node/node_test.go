...
func TestPodFitsCurrentNode(t *testing.T) {
	...
	for _, tc := range tests {
		actual := PodFitsCurrentNode(ctx, fakeClient, tc.pod, tc.node)
...
func TestPodFitsAnyOtherNode(t *testing.T) {
	// We must manually enable FeatureGate states because in these unit tests Kubernetes core components remain uninitialized
	utilfeature.DefaultMutableFeatureGate.Add(map[featuregate.Feature]featuregate.FeatureSpec{
		utils.PodOverhead:                   {Default: true, PreRelease: featuregate.Beta},
		utils.LocalStorageCapacityIsolation: {Default: true, PreRelease: featuregate.Beta},
	})
	...
	tests := []struct {
		...
		},
		{
			...
			nodes: []*v1.Node{
				...
				// Notice that this node has 5 CPU cores, the pod below requests 2 cores, and has CPU overhead of 1 cores, and the pod above requests 2 cores
	...
	for _, tc := range tests {
		fakeClient := &fake.Clientset{}
		fakeClient.Fake.AddReactor("list", "pods", func(action core.Action) (bool, runtime.Object, error) {
			return true, &v1.PodList{Items: tc.podsOnNodes}, nil
		})

		actual := PodFitsAnyOtherNode(ctx, fakeClient, tc.pod, tc.nodes)
		if actual != tc.success {
			t.Errorf("Test %#v failed", tc.description)
		}
	}
