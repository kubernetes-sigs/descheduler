...
func TestNewFramework(t *testing.T) {
	var seconds uint = 10
	cfg := v1alpha2.DeschedulerConfiguration{
	...
	if status := frmwrk.RunDeschedulePlugins(ctx, []*v1.Node{node1}); status != nil && status.Err != nil {
		t.Fatalf("Running deschedule plugins returned non-empty status: %v", status.Err)
	}

	if status := frmwrk.RunBalancePlugins(ctx, []*v1.Node{node1}); status != nil && status.Err != nil {
		t.Fatalf("Running balance plugins returned non-empty status: %v", status.Err)
	}
}
